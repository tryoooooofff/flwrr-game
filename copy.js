...
export const BIOME_SPAWN_RATES = {
    "Plain": {
        ...
        "Anthill": { weight: 5, minLevel: 2, maxLevel: 5 },
    ...
    "Desert": {
        ...
        "FireAntHole": { weight: 10, minLevel: 1, maxLevel: 5 }
    },
    ...
    "Ocean": {
        ...
        "CrabHole": { weight: 5, minLevel: 2, maxLevel: 5 }       // 蟹洞 - 稀有，会生成螃蟹
    },
    // ========== 🆕 下水道生物生成概率 ==========
    "Sewer": {
        // 基础生物
        ...
        "ManHole": { weight: 2, minLevel: 2, maxLevel: 5 },
...
const ITEM_IMAGE_URLS = {
    ...
    // 在 ITEM_IMAGE_URLS 中
    "TrashDigger egg": "images/TrashDigger egg.png",
    "Digger egg": "images/Digger egg.png",
    "MudDigger egg": "images/MudDigger egg.png",
    "Biologist egg": "images/Biologist egg.png"
};
...
export const ENEMY_ARMOR_CLASSES = {
    ...
    "C": ["WhiteBloodCell", "Rock","Anthill","Scallop","Bubble","PooStorm"],
    "D": ["Rat", "Roach", "StemCell",]
};
...
class ShopSystem {
    constructor(inventory, quickSlot) {
        ...
        // Player's Star count
        this.stars = 4750000; // Initial stars
...
class Petal {
    ...
    trySpawnDiggers(gameEnemies, playerWorldPos, hasDNA, diggerType) {
        ...
        // 根据 Digger 类型设置不同的生成数量
        let spawnCount = maxCount - currentCount;
        if (diggerType === "Biologist") {
            // Biologist 蛋生成2个
            spawnCount = Math.min(2, maxCount - currentCount);
        } else if (diggerType === "Digger") {
            spawnCount = Math.min(3, maxCount - currentCount);
        } else if (diggerType === "TrashDigger") {
            spawnCount = Math.min(2, maxCount - currentCount);
        } else if (diggerType === "MudDigger") {
            spawnCount = Math.min(1, maxCount - currentCount);
        }
...
// 修改 autoSave 方法
    ...
    dropCard(enemy) {
        ...
        // 判断是否为下水道生物
        const isSewerEnemy = ["ManHole","Rat", "Roach", "PooStorm"].includes(enemy.type);
        // 判断是否为 Eternal 生物
        ...
        if (enemy.type === "StemCell" || enemy.type === "RedBloodCell") {
            ...
            // 注意：这里不要 return，也不要放在 else if 中，让代码继续往下走
        }
        // === Cancer 细胞 DNA ===
        ...
        if (enemy.type === "Bush") {
            ...
            // 继续处理 Bush 的普通掉落 (Leaf, Root 等)
            // 移除 ENEMY_DROP_TABLE 的重新获取，使用上面定义的 dropItems
            ...
            return; // Bush 处理完直接返回，不需要再走下面的通用逻辑
        }

        // ==========================================
        // 3. 通用普通物品掉落逻辑 (包含 StemCell 的其他掉落)
        // ==========================================

        // 现在 StemCell 也会运行到这里！
        for (const itemType of dropItems) {
            if (itemType === "DNA") continue; // 跳过 DNA，因为上面已经掉过了
            ...
            for (let i = 0; i < dropCount; i++) {
                if (isEternal) {
                    ...
                    this._processSewerEggDrop(enemy, itemType, enemyRarity, false, i);
                } else {
                    const rarity = this.getRandomRarity(enemyRarity, RARITY_DROP_RATES);
                    this._spawnItemDrop(enemy, itemType, rarity, i);
                }
